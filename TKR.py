...
#-------(esko na churna)-----
def flight():
	print(f'{HUB}[=] Use Flight Mode After [{PORNX}60{HUB}] Second')
def print_colored_message(message):
    ...
    formatted_message = f'{random_color}[ERROR]'
...
def generate_user_agent():
    ...
    browsers = [
        ...
        "Mozilla/5.0 AppleWebKit/605.1.15 (KHTML, like Gecko) Version/15.0 Safari/605.1.15",
        "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
        "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:97.0) Gecko/20100101 Firefox/97.0",
    ]
    user_agent = f"Mozilla/5.0 ({random.choice(platforms)}) AppleWebKit/{random.uniform(500, 700):.1f} ({random.choice(browsers)})"
    return user_agent
def generate_custom_user_agents():
    custom_user_agents = [
        "Mozilla/5.0 (iPod; CPU iPhone OS 17_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) CriOS/120.0.6099.119 Mobile/15E148 Safari/604.1",
...
def generate_all_user_agents(num_user_agents):
    custom_user_agents = generate_custom_user_agents()
    all_user_agents = custom_user_agents + [generate_user_agent() for _ in range(num_user_agents)]
    return all_user_agents
def print_user_agents(user_agents):
    for index, user_agent in enumerate(user_agents, start=1):
        print(f"[{index}] {user_agent}")
def porn():
    clear()
    num_user_agents = int(input("Enter the number of user agents to generate: "))
    all_user_agents = generate_all_user_agents(num_user_agents)
    print_user_agents(all_user_agents)
#--------(logo)-------
logo=("""\033[1;97m
...
>>>   Facebook   :  Veer
>>>   Github     :  Unknown 
>>>   Tool Type  :  \x1b[1;91mAuto-Create
\x1b[1;97m>>>   Version    :  1.0
\33[1;37m----------------------------------------------""")
#--------(menu)------
def menu():
	clear()
	print('[1] Auto Acounts Create')
	print('[2] Make Ua for Update ')
	print('[3] Contect to owner')
	lines()
...
def fucker():
    clear()
    print('[•] Example: Khan007, Ali007, ahad123, etc')
    lines()
    love = input('[•] Put Your Password: ')
    lines()
    print('[•] Example: 1000, 2000, 10000, etc')
    lines()
    limit = int(input('[•] Put Your Limit: '))
    lines()
    print('[•] Put File Name You Save Create Ids')
    lines()
    fuckx = input('Put File Name: ')
    lines()
    ...
    def strt():
       try:
           ...
           import sys
           loop+=1
           sys.stdout.write(f'\r\r{print_colored_message("%s/%s")} %s\033[1;37m/\033[1;37m%s/\033[1;32m%s\033[1;37m/\033[1;31m%s\033[1;37m/\033[1;36m%s' % (loop, limit, ok, cp, ok1));sys.stdout.flush()
           requests=r.Session()
           from fake_email import Email
           mmail=Email().Mail()
           em=mmail['mail']
